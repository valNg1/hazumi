...
export default function Progression() {
  ...
  const [activeParcoursTab, setActiveParcoursTab] = useState<'kyu'>('kyu')
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <button
          onClick={() => navigate('/eleve/shiai')}
          className="bg-white rounded-xl border border-[#E5E5E5] p-6 hover:shadow-md hover:border-[#C41230] transition-all text-left"
        >
          <p className="text-3xl mb-2">🥊</p>
          <p className="font-semibold text-[#0A0A0A] mb-1">Shiai</p>
          <p className="text-xs text-[#999999]">Mes vidéos de combat</p>
        </button>

        <div className="bg-white rounded-xl border border-[#E5E5E5] p-6">
          <p className="text-3xl mb-2">🎌</p>
          <p className="font-semibold text-[#0A0A0A] mb-1">Judo-Ka</p>
          <p className="text-xs text-amber-600">Contenu en préparation</p>
        </div>

        <button
          onClick={() => setActiveParcoursTab('kyu')}
          className="bg-white rounded-xl border border-[#E5E5E5] p-6 hover:shadow-md hover:border-[#C41230] transition-all text-left"
        >
          <p className="text-3xl mb-2">🥋</p>
          <p className="font-semibold text-[#0A0A0A] mb-1">Kyu</p>
          <p className="text-xs text-[#999999]">Progression par grade</p>
        </button>
      </div>

      {/* Kyu details */}
      {activeParcoursTab === 'kyu' && (
            <div className="mt-8">
              <h2 className="text-xl font-bold text-[#0A0A0A] mb-6">🥋 Kyu — Ma progression par grade</h2>

              {/* Tabs for grade/playlist */}
              <div className="flex gap-1 bg-[#F5F5F5] p-1 rounded-lg w-fit mb-6">
                ...
                <button
                  ...
                  className={`px-4 py-2 text-xs rounded-md transition-all ${mode === 'playlist' ? 'bg-white text-[#0A0A0A] shadow-sm font-semibold' : 'text-[#999999] hover:text-[#666666]'}`}
                >
                  Mes playlists
                </button>
              </div>
