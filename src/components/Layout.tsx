...
export default function Layout() {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('hazumi:messages-read', handler)
    }
  }, [])
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5] flex flex-col">
      <header className="bg-[#0A0A0A] px-4 sm:px-6 flex items-stretch sticky top-0 z-40">
        <Link
          to="/"
          className="flex items-center gap-2 sm:gap-3 py-3 mr-4 sm:mr-8 flex-shrink-0 hover:opacity-80 transition-opacity"
        >
          <img
            src="/logo.png"
            alt="Hazumi"
        ...
        <nav className="hidden lg:flex items-stretch gap-0.5 flex-1 overflow-x-auto">
          {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              className={`flex items-center gap-1.5 px-3 xl:px-4 text-xs uppercase tracking-widest border-b-2 transition-colors whitespace-nowrap ${
                isNavActive(item, pathname)
                  ? '!text-white !border-[#C41230]'
            ...
            >
              ...
              {(item.match ?? []).includes('/eleve/messages') && (
        ...
        <div className="flex items-center gap-3 py-3 ml-auto flex-shrink-0">
          <button
            onClick={switchSpace}
            className="flex items-center gap-1.5 text-xs text-white/70 hover:text-white transition-colors border border-[#2A2A2A] hover:border-[#444444] rounded-full px-3 py-1.5"
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${
                space === 'eleve' ? 'bg-[#C41230]' : 'bg-blue-400'
              }`}
            />

            {space !== null
              ? SPACE_LABEL[space as keyof typeof SPACE_LABEL]
              : ''}
          </button>
        ...
        <button
          className="lg:hidden text-white ml-auto py-3 pl-3"
